...
import { spacing, type ThemeColors } from '../../../src/theme/mobile-theme'
import { createStyles } from '../../../src/screen-styles/tasks-screen-styles'
...
function getProviderOptions(colors: ThemeColors): PickerOption<TaskProvider>[] {
  return [
    {
      value: 'github',
      label: 'GitHub',
...
function githubProjectOptionColor(color: string | null | undefined, colors: ThemeColors): string {
  if (!color) {
...
function compareTasksByRepository(
  ...
  const aRepo = taskRepositoryMeta(a, reposById, colors)
  const bRepo = taskRepositoryMeta(b, reposById, colors)
...
export default function MobileTasksScreen() {
  ...
  const providerOptions = useMemo(
    () => getProviderOptions(colors).filter((option) => visibleProviders.includes(option.value)),
    [visibleProviders, colors]
  )
  ...
  const sortedItems = useMemo(() => {
    ...
    if (taskSort === 'repository') {
      next.sort((a, b) => compareTasksByRepository(a, b, reposById, colors))
    } else {
  ...
  const linearIssueSections = useMemo(
    ...
    [linearGroupBy, linearIssuesForView, linearOrderBy, colors]
  )
                  ...
                  {isGitHubPr ? (
                    <View style={styles.prSignalRow}>
                      ...
                      <View
                        style={[
                          styles.prSignalChip,
                          getPrSignalToneStyle(getGitHubPRSignalTone(item.source, 'review'), styles)
                        ]}
                      >
                        <Text style={styles.prSignalText} numberOfLines={1}>
                      ...
                      <View
                        style={[
                          styles.prSignalChip,
                          getPrSignalToneStyle(getGitHubPRSignalTone(item.source, 'checks'), styles)
                        ]}
                      >
                        <Text style={styles.prSignalText} numberOfLines={1}>
                      ...
                      <View
                        style={[
                          styles.prSignalChip,
                          getPrSignalToneStyle(getGitHubPRSignalTone(item.source, 'merge'), styles)
                        ]}
                      >
                        <Text style={styles.prSignalText} numberOfLines={1}>
                            ...
                            <View style={styles.issueTypeChipContent}>
                              <View
                                style={[
                                  styles.issueTypeDot,
                                  {
                                    backgroundColor: githubProjectOptionColor(
                                      issueType.color,
                                      colors
                                    )
                                  }
                              ...
                              <View style={styles.fileActionRow}>
                                ...
                                <View
                                  style={[
                                    ...
                                    getGitLabPipelineStatusStyle(job.status, styles)
                                  ]}
                                >
...
function getGitLabPipelineStatusStyle(status: string, styles: ReturnType<typeof createStyles>) {
  switch (status) {
    case 'success':
