...
export function MobileNativeChatQuestion({ question, onAnswer }: Props): React.JSX.Element {
  ...
  const answerSingle = (option: string): void => {
    onAnswer(formatQuestionAnswer(question, [option]))
  }
  ...
  const submitMulti = (): void => {
    ...
    onAnswer(formatQuestionAnswer(question, selected))
  }
  ...
  const submitFreeText = (): void => {
    ...
    onAnswer(formatQuestionAnswer(question, [trimmedFreeText]))
    setFreeText('')
  }

  const canSubmitMulti = selected.length > 0
  const canSendFreeText = trimmedFreeText.length > 0
