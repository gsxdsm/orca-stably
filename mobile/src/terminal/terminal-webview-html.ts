  ...
  function attachSurfaceEventHandlers(targetSurface) {
    ...
    targetSurface.addEventListener('touchmove', function(e) {
      ...
      if (e.touches.length === 2) {
        ...
        // Why: pan horizontally only when the scaled grid overflows the viewport
        // (wider than fit). Vertical always drives buffer scroll so scrollback
        // stays reachable at any text size — the two axes are independent.
